        ...
        <aside class="widget w-latest-news">
            ...
            <ul class="latest-news-list">
                @foreach ($trending as $vtrending)
                    @php
                        $vslug = \Illuminate\Support\Str::slug($vtrending->title);
                    @endphp
                    <li>
                        <article itemscope="" itemtype="http://schema.org/NewsArticle" class="latest-news-item">
                            <div class="post__date">
                                <time class="published" datetime="2018-03-08 12:00:00">
                                    <span class="number">{{ date('j', strtotime($vtrending->post_date)) }}</span>
                                    {{ date('M', strtotime($vtrending->post_date)) }},
                                    {{ date('Y', strtotime($vtrending->post_date)) }}
                                </time>
                            </div>
                            <a href="{{ url('/') }}/single/{{ $vtrending->id }}/{{ $vslug }}"
                                class="h6 post__title entry-title">{{ $vtrending->title }}</a>
                        </article>
                    </li>
                @endforeach
            </ul>
