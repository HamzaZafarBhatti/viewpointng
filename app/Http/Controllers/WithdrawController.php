...
class WithdrawController extends Controller
{
    ...
    public function ref_withdraw_log()
    {
        $data['title'] = 'Referral Balance Withdraw logs';
        $data['withdraw'] = Withdraw::with('user')->whereType(3)->latest('id')->get();
        return view('admin.withdrawal_ref.index', $data);
    }
    public function ref_withdraw_approve(Request $request)
    {
        // return $request;
        $data = Withdraw::findOrFail($request->id);
        $user = User::find($data->user_id);
        ...
        $user->update([
            'show_popup' => 1,
            'affliate_ref_balance' => $user->affliate_ref_balance + $remainder_amount
        ]);
        $res = $data->update([
            'status' => '1',
            'amount' => $payment_value
        ...
        if ($res) {
            Notification::create([
                'user_id' => $user->id,
                'title' => 'Referral Balance WITHDRAWAL - SUCCESSFUL  - NGN' . $data->amount,
                'msg' => 'Your Withdrawal Request from your Referral Balance has been APPROVED. Kindly POST your Payment Credit ALERT!',
                'is_read' => 0
            ]);
            return redirect()->route('admin.ref.withdraw_log')->with('success', 'Request was Successfully approved!');
        } else {
            return redirect()->route('admin.ref.withdraw_log')->with('alert', 'Problem With Approving Request');
        }
    }

    public function ref_withdraw_approved()
    {
        $data['title'] = 'Approved Referral Balance Withdraw';
        $data['withdraw'] = Withdraw::with('user')->whereType(3)->whereStatus('1')->latest('id')->get();
        // $data['withdraw'] = Withdraw::whereStatus(1)->orderBy('id', 'DESC')->get();
        return view('admin.withdrawal_ref.approved', $data);
    }

    public function ref_withdraw_approve_multi(Request $request)
    {
        // return $request;
    ...
    public function ref_withdraw_unpaid()
    {
        $data['title'] = 'Unpaid Referral Balance Withdraw';
        $data['withdraw'] = Withdraw::with('user')->whereStatus('0')->whereType(3)->latest('id')->get();
        return view('admin.withdrawal_ref.unpaid', $data);
    }

    public function ref_withdraw_delete($id)
    {
        $data = Withdraw::findOrFail($id);
        // return json_encode($data->status == '0');
        if ($data->status == '0') {
    ...
    public function ref_withdraw_decline($id)
    {
        $data = Withdraw::findOrFail($id);
        $user = User::find($data->user_id);
        $set = Setting::first();
        $res = $data->update(['status' => '2']);
        $user->update(['affliate_ref_balance' => $user->affliate_ref_balance + $data->amount]);
        // if ($set->email_notify == 1) {
        //     $temp = Etemplate::first();
        //     Mail::to($user->email)->send(new GeneralEmail($temp->esender, $user->username, 'Withdrawal request of ₦' . substr($data->amount, 0, 9) . ' has been declined<br>Thanks for working with us.', 'Withdraw Request has been declined'));
        // }
        if ($res) {
            Notification::create([
                'user_id' => $user->id,
                'title' => 'Referral Balance WITHDRAWAL - DECLINED - NGN' . $data->amount,
                'msg' => 'Your Withdrawal Request from your Referral Balance has been DECLINED - Please update your Bank account and try again or contact Rubic Network Support.',
                'is_read' => 0
            ]);
            return back()->with('success', 'Request was Successfully declined!');
        } else {
    ...
    public function ref_withdraw_declined()
    {
        $data['title'] = 'Declined Withdraw';
